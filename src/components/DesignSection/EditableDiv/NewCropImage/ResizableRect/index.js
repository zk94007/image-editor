...
export default class ResizableRect extends Component {
  ...
  handleResize = (length, alpha, rect, type, isShiftKey, clientX, clientY) => {
    if (!this.props.onResize) return
    length /= this.props.zoomLevel;
    ...
    this.props.onResize(centerToTL({ centerX, centerY, width, height, rotateAngle }), isShiftKey, type, clientX, clientY)
  }
  ...
  render () {
    const {
      top, left, width, height, rotateAngle, parentRotateAngle, position, overflow, display, zoomable, rotatable,
      onRotate, onResizeStart, onResizeEnd, onRotateStart, onRotateEnd, onDragStart, onDragEnd
    } = this.props

    const styles = tLToCenter({ top, left, width, height, rotateAngle, position, overflow, display })
